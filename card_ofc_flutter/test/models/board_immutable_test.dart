...
void main() {
  group('OFCBoard Immutable API', () {
    ...
    test('T4: Deck(seed: 42).deal(5) 두 번 실행 동일 결과', () {
      final deck1 = Deck(seed: 42);
      final deck2 = Deck(seed: 42);
      final hand1 = deck1.deal(5);
      final hand2 = deck2.deal(5);
      expect(hand1.length, 5);
      expect(hand2.length, 5);
      for (int i = 0; i < 5; i++) {
        expect(hand1[i].rank, hand2[i].rank);
        expect(hand1[i].suit, hand2[i].suit);
      }
    });
